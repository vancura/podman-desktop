...
<svelte:window />
<nav
  class="group w-leftnavbar {minNavbarWidth} shrink-0 flex flex-col bg-[var(--pd-global-nav-bg)] border-[var(--pd-global-nav-bg-border)] border-r-[1px]"
  ...
  <div bind:this={outsideWindow}>
    <NavItem href="/accounts" tooltip="" ariaLabel="Accounts" bind:meta={meta} onClick={(event): void => authActions?.onButtonClick(event)}>
